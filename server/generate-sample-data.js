...
// Maharashtrian Instructor Names
...
// Instructor names in sampleCourses match the Maharashtrian names
const sampleCourses = [
  {
  ...
  try {
    ...
    const instructorProfiles = await Profile.insertMany(
      sampleInstructors.map(instructor => ({
        gender: (instructor.firstName === "Rucha" || instructor.firstName === "Sneha") ? "Female" : "Male",
        dateOfBirth: "1990-01-01",
        about: instructor.bio,
        contactNumber: "+91-9876543210", // Retaining string format as per your schema
      }))
    );
    ...
    const instructors = await User.insertMany(
      sampleInstructors.map((instructor, index) => ({
        ...
        courses: [], // Initializing as empty array, as per User schema
        courseProgress: [], // Initializing as empty array, as per User schema
      }))
    );
    ...
    const studentProfiles = await Profile.insertMany(
      sampleStudents.map(student => ({
        gender: (student.firstName === "Amruta" || student.firstName === "Vaishnavi" || student.firstName === "Pooja") ? "Female" : "Male",
    ...
    for (let i = 0; i < sampleCourses.length; i++) {
      ...
      // Debugging logs to help identify the problem - KEEP THESE FOR THE NEXT RUN
      console.log(`\nProcessing Course: ${courseData.courseName}`);
      console.log(`Expected Instructor Name from courseData: "${courseData.instructor}"`);
      console.log(`Found Instructor (if any): ${instructor ? instructor.firstName + ' ' + instructor.lastName : 'NOT FOUND'}`);
      console.log(`Instructor object itself:`, instructor); // Crucial: Check if this is truly an object
      console.log(`Instructor.courses property BEFORE push:`, instructor ? instructor.courses : 'Instructor is undefined'); // Crucial: Check its type
      console.log(`Expected Category Name from courseData: "${courseData.category}"`);
      console.log(`Found Category (if any): ${category ? category.name : 'NOT FOUND'}`);
      console.log(`Tags found for this course (names): ${tags.map(t => t.name).join(', ')}`);


      if (!instructor) {
        console.error(`ERROR: Instructor "${courseData.instructor}" not found for course "${courseData.courseName}". Skipping this course.`);
        continue;
      }
      if (!category) {
        console.error(`ERROR: Category "${courseData.category}" not found for course "${courseData.courseName}". Skipping this course.`);
        continue;
      }
      ...
      const course = await Course.create({
        ...
        tag: tags.map(tag => tag._id), // Correctly storing tag _ids
      ...
      // This is the line that's causing issues. The logs above it should tell us why.
      ...
      for (const tag of tags) {
        ...
        if (foundTag) {
          foundTag.courses.push(course._id);
          await foundTag.save();
        }
      }
    }
    ...
    for (let i = 0; i < students.length; i++) {
      ...
      const enrolledCourses = createdCourses.slice(0, Math.floor(Math.random() * 3) + 1);
